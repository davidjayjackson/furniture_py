...
# Load the only sheet as a DataFrame
#sales = sales_data.parse(0)
...
```
### 1. Aggregate total sales, profit, and quantity by month
...
plt.show()
```
### 2. Breakdown of sales by region and market segment
...
plt.show()
```
### 3. KPIs for AOV, CLV, and Profit Margin and Average Order Value (AOV)
...
```
### 4. Sales growth/decline MoM and YoY
...
print(monthly_summary[['year_month', 'sales_growth_mom']])

print("Year-over-Year Sales Growth:")
print(monthly_summary[['year_month', 'sales_growth_yoy']])
...
plt.show()
```
### 5: Visual report of sales trends over time

```{python}
plt.figure(figsize=(14, 7))
plt.plot(monthly_summary['year_month'].astype(str), monthly_summary['total_sales'], label='Total Sales', marker='o')
plt.plot(monthly_summary['year_month'].astype(str), monthly_summary['profit'], label='Total Profit', marker='o')
plt.plot(monthly_summary['year_month'].astype(str), monthly_summary['quantity'], label='Quantity Sold', marker='o')
